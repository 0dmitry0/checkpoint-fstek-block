    ...
    with APIClient(client_args) as client: 
        ...
        if client.check_fingerprint() is False:
            ...
            if function_flag == 'add-host':
                ...
                print(add_host)
            elif function_flag == 'add-rule':
                add_rule = client.api_call('add-access-rule', {'name': 'fstek block', 'layer': layer, 'position': 'bottom', 'source': 'fstek'})
                print (add_rule)
...
class rule:
    ...
    def add_rule(self):
         main_api_func('add-rule', self.ip, self.name, self.ips_with_names)
